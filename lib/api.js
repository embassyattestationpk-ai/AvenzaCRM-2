...
export const api = {
  ...
  getCaseBalance: (id) => get('getCaseBalance', { id }),
  writeOffCase: (data) => post('writeOffCase', data),
  ...
  // Multiple-board cases (legacy — superseded by Services_JSON, kept for old cases)
  ...
  // PHASE 16 (Part A) — services/per-document tracking
  getCaseServices: (id) => get('getCaseServices', { id }),
  updateDocumentStatus: (data) => post('updateDocumentStatus', data),
  getReceiptPdf: (caseId) => get('getReceiptPdf', { caseId }),
  ...
  // PHASE 16 (Part B) — editable Service -> Document Type access mapping
  getServiceDocTypeMap: () => get('getServiceDocTypeMap'),
  updateServiceDocTypeMap: (data) => post('updateServiceDocTypeMap', data),

  // PHASE 16 (Part B) — client-side standard rate table
  getClientRates: (params) => get('getClientRates', params),
  addClientRate: (data) => post('addClientRate', data),
  updateClientRate: (data) => post('updateClientRate', data),
  deleteClientRate: (id) => post('deleteClientRate', { id }),
  upsertClientRate: (data) => post('upsertClientRate', data),

  // PHASE 16 (Part B) — richer daily analytics report (test-send button)
  sendDailySummaryReportNow: () => post('sendDailySummaryReportNow', {}),
};
